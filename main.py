...
def find_pollen_value(pollen):
    for item in pollen:
        ...
        for inner_item in item['concentrations']:
            define_pollen_value(request_concentrations(inner_item)['value'], request_allergen(request_concentrations(inner_item)))
...
def request_pollen():
    ...
    pollen = requests.get(f"http://polen.sepa.gov.rs/api/opendata/pollens/?location={location}&date_after={define_date()}")
    pollen = pollen.json()
